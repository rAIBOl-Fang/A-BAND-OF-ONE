...
void exposes_the_guard_or_timeout_as_the_next_deadline() {
  PlatformSelectionController controller;
  std::uint32_t deadline = 0;
  assert(!controller.next_deadline_ms(&deadline));

  controller.arm(1'000, 10'000);
  assert(controller.next_deadline_ms(&deadline));
  assert(deadline == 11'000);

  assert(controller.handle_event(InputId::Key1, InputPhase::Pressed, 1'100).consumed);
  assert(controller.handle_event(InputId::Key1, InputPhase::Released, 1'120).consumed);
  assert(controller.next_deadline_ms(&deadline));
  assert(deadline == 1'220);
}
...
int main() {
  ...
  exposes_the_guard_or_timeout_as_the_next_deadline();
  return 0;
}
