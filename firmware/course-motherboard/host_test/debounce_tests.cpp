...
void pending_transition_exposes_its_exact_deadline() {
  ai_keyboard::DebouncedInput input(5, 3);
  input.reset(false, 0xFFFF'FFF0U);

  assert(!input.transition_pending());
  assert(!input.update(true, 0xFFFF'FFFDU).changed);
  assert(input.transition_pending());

  std::uint32_t deadline = 0;
  assert(input.next_transition_deadline_ms(true, &deadline));
  assert(deadline == 2U);
  assert(!input.next_transition_deadline_ms(false, &deadline));

  const auto accepted = input.update(true, 2U);
  assert(accepted.changed);
  assert(!input.transition_pending());
  assert(!input.next_transition_deadline_ms(true, &deadline));
}
...
int main() {
  ...
  pending_transition_exposes_its_exact_deadline();
  return 0;
}
