...
constexpr std::uint32_t kAudioRuntimeMaxStreamMs = 300000;
...
// I2S；更长的中断会显式表现为序号缺口，由 App 保留时间轴。
    ...
    if (!config.enabled) {
      ...
      if (endpoint_changed) {
        ...
        if (session_lifecycle_.active()) {
          ...
          ESP_LOGI(kTag,
                   "audio config endpoint changed while streaming; reconnect scheduled");
        }
      }
      if (init_state_ == InitState::Ready &&
...
void KeyboardAudioLink::set_work_ready_callback(WorkReadyCallback callback,
  ...
  work_ready_callback_context_ = context;
  unlock();
}
...
void KeyboardAudioLink::start_stream(const char* reason, std::uint64_t session_id) {
  ...
  if (audio_io_arbiter_ != nullptr &&
    ...
    // Reversible quiesce admits the request and atomically cancels that sleep
    // attempt. A rejection therefore means the irreversible audio terminal
    // cutoff has already won; this session must not be resurrected.
    session_lifecycle_.finish(generation, "deep_sleep_terminal");
    ...
    diagnostics_.stop_reason = "deep_sleep_terminal";
    unlock();
    ESP_LOGI(kTag,
             "keyboard mic start rejected after deep-sleep terminal cutoff session=%llu generation=%lu",
             static_cast<unsigned long long>(session_id),
...
void KeyboardAudioLink::run_control_channel() {
  ...
  for (;;) {
    ...
    if (sock < 0) {
      ...
      ESP_LOGI(kTag, "audio control channel ready");
    }
    ...
    if (!dest_valid || dest_host != config.host || dest_port != config.port) {
      ...
      if (gai != 0 || result == nullptr || result->ai_addr == nullptr ||
        ...
        ESP_LOGW(kTag, "audio control resolve failed gai=%d", gai);
...
void KeyboardAudioLink::request_wifi_release_for_deep_sleep() {
  ...
  unlock();
  if (requested && control_task != nullptr) {
    xTaskNotifyGive(control_task);
  }
}
...
void KeyboardAudioLink::notify_work_ready() {
  WorkReadyCallback callback = nullptr;
  void* context = nullptr;
  lock();
  callback = work_ready_callback_;
  context = work_ready_callback_context_;
  unlock();
  if (callback != nullptr) {
    callback(context);
  }
...
void KeyboardAudioLink::note_remote_work_ready() {
  note_network_activity();
  notify_work_ready();
}
...
bool KeyboardAudioLink::management_work_pending() const {
  lock();
  const bool pending = !pending_config_json_.empty();
  unlock();
  return pending;
}
...
bool KeyboardAudioLink::wifi_active_or_streaming() const {
  ...
  unlock();
  return wifi_connected || streaming || wifi_control_active;
}

bool KeyboardAudioLink::shutdown_wifi_for_deep_sleep() {
  WifiOpGuard wifi_guard(wifi_op_mutex_);
  lock();
  ...
  unlock();
  if (!safe) {
    ESP_LOGE(kTag, "terminal Wi-Fi shutdown rejected: release contract incomplete");
    return false;
  }
  if (!started) {
    return true;
  ...
  if (err != ESP_OK && err != ESP_ERR_WIFI_NOT_STARTED) {
    ESP_LOGE(kTag,
             "terminal Wi-Fi shutdown failed: %s",
             esp_err_to_name(err));
    return false;
  }
  lock();
  ...
  xEventGroupClearBits(wifi_events_, kWifiConnectedBit);
  xEventGroupSetBits(wifi_events_, kWifiDisconnectedBit);
  ESP_LOGI(kTag, "terminal Wi-Fi shutdown complete for Deep Sleep");
  return true;
}
...
void KeyboardAudioLink::run_audio_stream(std::uint32_t generation,
  ...
  while (should_run_stream(generation)) {
    ...
    if (elapsed_ms >= kAudioRuntimeMaxStreamMs) {
    ...
    if (sent < 0) {
      ...
      if (err != ESP_OK || !should_run_stream(generation)) {
...
void KeyboardAudioLink::preconnect_wifi(const char* reason) {
  ...
  ESP_LOGI(kTag,
           "keyboard mic Wi-Fi preconnect requested reason=%s",
           reason == nullptr ? "" : reason);
}
...
esp_err_t KeyboardAudioLink::prepare_wifi_connection(const KeyboardAudioConfig& config,
...

  {
    ...
    ESP_LOGI(kTag,
             "keyboard mic connecting Wi-Fi reason=%s reuse_config=%d wait=%d",
             reason == nullptr ? "" : reason,
