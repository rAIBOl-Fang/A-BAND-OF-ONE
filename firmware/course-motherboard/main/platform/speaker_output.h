...
class SpeakerOutput {
 public:
  ...
  // arbitration path used by a bank-backed user asset. A non-zero sustain
  // loop window keeps the worker decoding until cancel_active() (or an
  // arbiter preemption) stops it, so a held key holds its note.
  ...
  // Starts a continuous PCM stream on the same worker/I2S/power path as
  // factory assets. The provider owns the voice state and fills one 10 ms
  // mono block per call; finished means that block is the final release block.
  bool request_voice_stream(VoiceFrameProvider provider, void* context);
#endif
  ...
  // Stops the currently active playback at the next frame boundary (about
  // 10 ms). Safe to call from the same task that owns request/poll when a
  // note-off must truncate a sustained sound; the worker skips the normal
  // drain and releases I2S immediately, exactly like a microphone preemption.
  void cancel_active();
 ...
 private:
...
#if defined(EASY_INPUT_SPEAKER_ASSETS_PRODUCT)
  bool submit_open_request(std::uint32_t generation, RequestKind kind);
  esp_err_t prepare_voice_first_frame(std::uint32_t generation,
                                      std::int16_t* output,
                                      std::size_t output_capacity,
                                      bool* finished);
  esp_err_t play_voice_frames(std::uint32_t generation,
                              std::int16_t* frame,
  ...
  VoiceFrameProvider voice_provider_ = nullptr;
