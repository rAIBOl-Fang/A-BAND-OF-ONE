...
// rail is stable. Block this task instead of keeping the CPU busy during the
// rail load step. The extra tick guarantees at least the requested interval
// regardless of where the call lands inside the current scheduler tick.
constexpr std::uint32_t kPeripheralPowerSettleMs = 50U;
constexpr std::uint64_t kMillisecondsPerSecond = 1000U;

constexpr TickType_t peripheral_power_settle_ticks() {
  // vTaskDelay(N) can be almost one tick shorter depending on the phase of
  // the current tick. Round the requested duration up, then add one complete
  // tick so the physical blocked interval is never shorter than 50 ms.
  const auto rounded_up_ticks =
      (static_cast<std::uint64_t>(kPeripheralPowerSettleMs) *
           configTICK_RATE_HZ +
       kMillisecondsPerSecond - 1U) /
      kMillisecondsPerSecond;
  return static_cast<TickType_t>(rounded_up_ticks + 1U);
}
...
// This is the sole physical GPIO8 write site. It is intentionally usable
// before the pin becomes an output so begin_awake() can preload a known-safe
// latch value without claiming that the external rail is already configured.
esp_err_t write_power_enable_latch(bool enabled) {
...
esp_err_t PeripheralPowerController::begin_awake() {
  ...
  // gpio_config(..., OUTPUT) enables the existing GPIO_OUT latch but does not
  // initialize it. Preload inactive before enabling the output so every reset
  // path starts from a deterministic rail-off state.
  const esp_err_t preload_err = write_power_enable_latch(false);
  if (preload_err != ESP_OK) {
    ESP_LOGE(kTag,
             "GPIO%d inactive latch preload failed: %s",
  ...
  // every Awake task wait; the board pulldown would otherwise turn it off.
  ...
  ready_ = true;
  ESP_LOGI(kTag,
           "shared rail ready GPIO%d settle_min_ms=%lu wait=scheduler_blocked awake_hold=active",
           static_cast<int>(ai_keyboard::kPeripheralPowerEnablePin),
           static_cast<unsigned long>(kPeripheralPowerSettleMs));
  return ESP_OK;
}
...
esp_err_t PeripheralPowerController::set_power_enabled(bool enabled) {
  ...
  const auto level = static_cast<std::uint8_t>(
      enabled == (ai_keyboard::kPeripheralPowerEnableActiveLevel != 0));
  ESP_LOGI(kTag,
           "set shared rail GPIO%d level=%u enabled=%d",
           static_cast<int>(ai_keyboard::kPeripheralPowerEnablePin),
           static_cast<unsigned>(level),
           enabled ? 1 : 0);
  const esp_err_t err = write_power_enable_latch(enabled);
  if (err != ESP_OK) {
    ESP_LOGE(kTag,
