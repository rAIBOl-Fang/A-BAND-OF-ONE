...
bool SpeakerAssetsSupervisor::wifi_carrier_started() const {
  return initialized_ &&
         wifi_started_.load(std::memory_order_acquire);
}

std::uint32_t SpeakerAssetsSupervisor::boot_progress_generation() const {
  return store_runner_.completed_unit_generation();
}

ai_keyboard::OwnerServiceSchedule
SpeakerAssetsSupervisor::work_schedule(
  ...
  mailbox_pending =
      mailbox_.close_size() != 0U || mailbox_.data_size() != 0U;
  portEXIT_CRITICAL(&mailbox_mux_);

  const bool core_lifecycle_pending = core_.lifecycle_size() != 0U;
  const bool core_ingress_pending = core_.ingress_size() != 0U;
  const bool core_records_pending =
      core_lifecycle_pending || core_ingress_pending;
  const bool core_action_pending = core_.action_pending();
  const bool store_active = store_runner_.job_active();
  const bool reply_capacity_available =
      core_.reply_size() <
      speaker_assets::kSpeakerAssetsRuntimeReplyCapacity;
  const bool reply_pending =
      core_.reply_size() != 0U || queued_reply_sequence_ != 0U;

  schedule.outstanding =
      mailbox_pending || core_records_pending || core_action_pending ||
      store_active || reply_pending;

  // Importing one callback record is always bounded and locally runnable.
  // Core work shares the Store runner with Boot resolution, so it advances
  // only while BootRead is Idle. A running Store unit is callback-driven;
  // re-polling it before the worker notification would be a busy loop.
  schedule.runnable_now =
      mailbox_pending ||
      (boot_read_state_ == BootReadState::Idle &&
       (core_lifecycle_pending ||
        (core_ingress_pending && !core_action_pending &&
         reply_capacity_available) ||
        (core_action_pending && resource_steps_allowed && !store_active &&
         reply_capacity_available)));

  // Partial assembly and authenticated session leases are protocol business
  // deadlines. They remain paused behind the exclusive Boot read, then become
  // immediately due if that lifecycle finishes after their deadline.
  if (boot_read_state_ == BootReadState::Idle) {
    schedule.deadline_armed =
        core_.next_deadline_ms(now_ms, &schedule.deadline_ms);
  }
  return schedule;
}
...
SpeakerAssetsSupervisor::boot_work_schedule() const {
  ...
  switch (boot_read_state_) {
    case BootReadState::Idle:
      if (boot_start_pending_) {
        schedule.outstanding = true;
        // A shared protocol Store job wakes the owner when it completes. Once
        // poll() retires that exact job, no further callback is forthcoming,
        // so the retained Boot intent becomes synchronously runnable.
        schedule.runnable_now = !store_runner_.job_active();
      }
      return schedule;
    case BootReadState::Preparing:
    case BootReadState::ReleasePending:
      // The Store worker notifies the platform owner when it requests another
      // bounded permit or publishes its completion. Re-running poll() before
      // that notification cannot make progress and would busy-spin Awake.
      schedule.outstanding = true;
      return schedule;
    case BootReadState::Ready:
    case BootReadState::FactoryDefault:
    case BootReadState::Unavailable:
      // poll() consumed the worker completion after service_speaker() already
      // ran in this owner pass. The terminal result is now synchronously
      // consumable, so schedule exactly one immediate continuation.
      schedule.outstanding = true;
      schedule.runnable_now = true;
      return schedule;
    case BootReadState::LeaseOutstanding:
      schedule.outstanding = true;
      // Normal playback waits for SpeakerOutput's completion notification. A
      // queued release is runnable only when the shared Store slot is free;
      // otherwise that slot's worker completion will notify the owner.
      schedule.runnable_now =
          boot_release_requested_ && !store_runner_.job_active();
      return schedule;
  }
  return schedule;
}
  ...
  if (boot_read_state_ == BootReadState::Idle) {
    ...
    if (started ==
      ...
      return SpeakerAssetsBootPlaybackResult::Unavailable;
        ...
        started ==
        speaker_assets::SpeakerAssetsInternalJobStartResult::Busy) {
      // A protocol Store job owns the shared slot. Keep the Boot intent
      // latched; its completion notification plus boot_work_schedule() will
      // re-enter the owner exactly once when the slot becomes idle.
      boot_start_pending_ = true;
    }
...
void SpeakerAssetsSupervisor::advance_boot_read_job() {
  if (boot_read_state_ == BootReadState::Idle &&
    ...
    if (started ==
      ...
      ESP_LOGE(kTag, "failed to resume Boot sound read job");
    }
    return;
  }
...
void SpeakerAssetsSupervisor::poll(
  ...
  if (boot_read_state_ == BootReadState::Idle) {
    ...
    // run_core_steps() may have consumed the completion of the protocol Store
    // job that previously blocked ResolveBoot. Start the retained Boot intent
    // in this same owner pass instead of waiting for a notification that can
    // no longer occur.
    advance_boot_read_job();
  }
  offer_usb_reply();
