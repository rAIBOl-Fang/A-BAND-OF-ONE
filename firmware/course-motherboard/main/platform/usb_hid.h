...
class UsbHidTransport {
 public:
  ...
  void on_tinyusb_resume();
  // TinyUSB endpoint callbacks publish transfer completion/failure and wake
  // the single firmware owner. They never drain platform queues themselves.
  void on_hid_report_complete();
  void on_hid_report_failed();
 ...
 private:
  ...
  bool send_hotkey_app_command(const std::string& hotkey,
                               bool pressed,
                               std::uint32_t expected_epoch);
  bool send_host_action_report(const std::string& value,
                               std::uint32_t expected_epoch);
  ...
  std::atomic<bool> report_in_flight_{false};
