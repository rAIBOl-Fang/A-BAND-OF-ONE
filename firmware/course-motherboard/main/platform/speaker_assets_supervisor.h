...
class SpeakerAssetsSupervisor final {
 public:
  ...
  bool ready() const;
  bool wifi_carrier_started() const;
  std::uint32_t boot_progress_generation() const;
 ...
 private:
  ...
  // ResolveBoot is an owner intent, not a periodic poll. If the shared Store
  // slot is occupied by protocol work, retain that intent until poll() retires
  // the exact job and can deterministically start the Boot read.
  bool boot_start_pending_ = false;
