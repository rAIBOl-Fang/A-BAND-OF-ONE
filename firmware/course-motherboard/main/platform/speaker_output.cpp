...
bool SpeakerOutput::submit_open_request(std::uint32_t generation,
                                        RequestKind kind) {
  reset_probe_run(generation);
  if (!audio_io_arbiter_->try_begin_speaker(generation)) {
    if (kind == RequestKind::Asset) {
...
bool SpeakerOutput::request_asset(
  ...
  if (!submit_open_request(ticket.generation, RequestKind::Asset)) {
    return false;
  }
  ESP_LOGI(kTag,
...
bool SpeakerOutput::request_embedded_asset(
    ...
    std::uint16_t loop_start_frame,
    std::uint16_t loop_end_frame) {
  if (!ready() || worker_task_ == nullptr || tx_channel_ == nullptr) {
    return false;
  }
  const auto ticket = playback_.request();
  ...
  if (asset_decoder_.open_embedded(
          encoded, encoded_bytes, loop_start_frame, loop_end_frame) !=
      speaker_assets::SoundAssetReadResult::Ok) {
    playback_.cancel(ticket.generation);
    playback_.mark_drained(ticket.generation);
    return false;
...
bool SpeakerOutput::request_voice_stream(VoiceFrameProvider provider,
                                         void* context) {
  if (!ready() || worker_task_ == nullptr || tx_channel_ == nullptr ||
      provider == nullptr) {
    return false;
  ...
  voice_provider_ = provider;
  voice_context_ = context;
  if (!submit_open_request(ticket.generation, RequestKind::Voice)) {
    voice_provider_ = nullptr;
    voice_context_ = nullptr;
    return false;
  }
  ESP_LOGI(kTag,
           "voice stream requested generation=%lu",
           static_cast<unsigned long>(ticket.generation));
  return true;
}
#endif
...
ai_keyboard::SpeakerPlaybackEvents SpeakerOutput::poll(
  ...
  ai_keyboard::SpeakerPlaybackEvents events;
  if (!ready_) {
  ...
  if (generation != 0) {
    ...
    // Acquire terminal first. If it is current, that release synchronizes all
    // preceding worker writes, including publish_started(). Loading started
    // afterwards therefore cannot lose the first-PCM edge when a short sound
    // starts and completes between two owner polls.
    ...
    auto result = WorkerResult::None;
    auto terminal_result = ai_keyboard::SpeakerPlaybackResult::None;
    if (completed == generation) {
      result = static_cast<WorkerResult>(
          completed_result_.load(std::memory_order_relaxed));
      switch (result) {
        case WorkerResult::Succeeded:
          terminal_result = ai_keyboard::SpeakerPlaybackResult::Succeeded;
          break;
        case WorkerResult::Cancelled:
          terminal_result = ai_keyboard::SpeakerPlaybackResult::Cancelled;
          break;
        case WorkerResult::Failed:
        case WorkerResult::None:
          terminal_result = ai_keyboard::SpeakerPlaybackResult::Failed;
          break;
      }
    }
    events = playback_.consume_worker_events(
        started, completed, terminal_result);
    if (events.terminal()) {
...
void SpeakerOutput::run() {
  while (true) {
    ...
    if (request_kind == RequestKind::Voice) {
      voice_provider_ = nullptr;
      voice_context_ = nullptr;
    }
#endif
...
SpeakerOutput::WorkerResult SpeakerOutput::play_sound(
  ...
  const bool voice_request = request_kind == RequestKind::Voice;
  bool first_voice_finished = false;
  if (asset_request) {
    ...
    err = prepare_voice_first_frame(
        generation, frame.data(), frame.size(), &first_voice_finished);
    if (err != ESP_OK) {
      return cancelled(generation) ? WorkerResult::Cancelled
                                   : WorkerResult::Failed;
    }
  ...
  if (asset_request) {
    ...
    err = first_voice_finished
              ? ESP_OK
              : play_voice_frames(generation, frame.data(), frame.size());
  } else
#endif
  {
#if defined(EASY_INPUT_SPEAKER_OPUS_DIAGNOSTIC)
...
#if defined(EASY_INPUT_SPEAKER_ASSETS_PRODUCT)
esp_err_t SpeakerOutput::prepare_voice_first_frame(
    std::uint32_t generation,
    std::int16_t* output,
    std::size_t output_capacity,
    bool* finished) {
  if (output == nullptr || finished == nullptr ||
      output_capacity < kSamplesPerFrame || voice_provider_ == nullptr) {
    return ESP_ERR_INVALID_ARG;
  }
  *finished = false;
  if (!voice_provider_(voice_context_, output, kSamplesPerFrame, finished)) {
    ESP_LOGW(kTag,
             "voice first frame render failed generation=%lu",
             static_cast<unsigned long>(generation));
...
esp_err_t SpeakerOutput::play_voice_frames(
    std::uint32_t generation,
    std::int16_t* frame,
  ...
  while (!cancelled(generation)) {
    bool finished = false;
    if (!voice_provider_(voice_context_, frame, kSamplesPerFrame, &finished)) {
      return ESP_ERR_INVALID_RESPONSE;
    }
    const auto write_result = write_samples(frame, frame_capacity);
    if (write_result != ESP_OK || finished) {
      return write_result;
    }
  }
  return ESP_OK;
