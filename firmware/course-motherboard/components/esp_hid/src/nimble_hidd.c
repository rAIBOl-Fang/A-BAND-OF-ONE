...
#include "easy_input_ble_svc_hid_writer.h"
...
static void nimble_report_write_cb(uint16_t conn_handle, uint16_t attr_handle,
...
struct esp_ble_hidd_dev_s {
    ...
    uint8_t                     devices_len;
    uint8_t                     direct_feature_report_id;
    easy_input_hidd_feature_write_handler_t direct_feature_handler;
    void                       *direct_feature_context;
};
...
static int nimble_hidd_dev_deinit(void *devp)
{
    ...
    easy_input_ble_svc_hid_register_report_write_cb(NULL);
    ble_svc_hid_register_char_write_cb(NULL);
    dev->direct_feature_handler = NULL;
    dev->direct_feature_context = NULL;
    unlock_hidd();
    unlock_owner_gate();
    ...
    if (dev->event_loop_handle != NULL) {
        const esp_err_t stop_post_err = esp_event_post_to(
                dev->event_loop_handle, ESP_HIDD_EVENTS,
                ESP_HIDD_STOP_EVENT, NULL, 0, 0);
...
esp_err_t easy_input_hidd_feature_write_handler_set(
    ...
    void *context)
{
    if (dev == NULL || handler == NULL || report_id == 0) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    s_dev->direct_feature_report_id = report_id;
    s_dev->direct_feature_handler = handler;
    s_dev->direct_feature_context = context;
    unlock_hidd();
    return ESP_OK;
}
...
esp_err_t easy_input_hidd_event_plane_shutdown(esp_hidd_dev_t *dev)
{
    ...
    wait_event_posts = begin_event_post_shutdown();
    event_loop = s_dev->event_loop_handle;
    s_dev->event_loop_handle = NULL;
    s_dev->direct_feature_handler = NULL;
    s_dev->direct_feature_context = NULL;
    unlock_hidd();

    /* A post reference protects event_loop until esp_event_post_to() returns.
     * esp_event_loop_delete() then takes the loop mutex, which the dispatcher
     * holds across each handler, providing the handler-completion barrier. */
    wait_for_event_posts(wait_event_posts);
                                   ...
                                   const uint8_t *data, uint16_t len)
{
    ...
    uint8_t usage = 0;
    uint8_t map_index = 0;
    easy_input_hidd_feature_write_handler_t direct_feature_handler = NULL;
    void *direct_feature_context = NULL;
    ...
    if (report_type == ESP_HID_REPORT_TYPE_FEATURE &&
            report_id == s_dev->direct_feature_report_id &&
            s_dev->direct_feature_handler != NULL) {
        direct_feature_handler = s_dev->direct_feature_handler;
        direct_feature_context = s_dev->direct_feature_context;
        const bool can_call = acquire_event_post_ref_locked(
                s_dev->event_loop_handle, &post_ref);
        unlock_hidd();
        if (can_call) {
            direct_feature_handler(
...
esp_err_t esp_ble_hidd_dev_init(esp_hidd_dev_t *dev_p, const esp_hid_device_config_t *config, esp_event_handler_t callback)
{
    ...
    easy_input_ble_svc_hid_register_report_write_cb(nimble_report_write_cb);
