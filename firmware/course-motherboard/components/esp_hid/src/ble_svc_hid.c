...
static easy_input_ble_svc_hid_report_write_cb_t s_conn_report_write_cb;
    ...
    for (int instance = 0; instance < instances; instance++) {
        ...
        case BLE_SVC_HID_CHR_UUID16_BOOT_KBD_OUT:
            ...
            if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR) {
                ...
                if (rc == 0 && conn_handle != BLE_HS_CONN_HANDLE_NONE &&
                        s_conn_report_write_cb) {
                    s_conn_report_write_cb(conn_handle, attr_handle,
                                           BLE_SVC_HID_RPT_TYPE_OUTPUT, 0,
                                           &hid_instances[instance].kbd_out_rpt,
                                           out_rpt_len);
                }
                return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
        ...
        case BLE_SVC_HID_CHR_UUID16_RPT:
            ...
            if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR) {
                ...
                if (conn_handle != BLE_HS_CONN_HANDLE_NONE &&
                        s_conn_report_write_cb) {
                    s_conn_report_write_cb(conn_handle, attr_handle, rpt->type,
                                           rpt->id, rpt->data, rpt->len);
                }
                if (ctxt->chr->flags & BLE_GATT_CHR_F_NOTIFY) {
                    ble_gatts_chr_updated(*(ctxt->chr->val_handle));
                }
