...
bool PlatformSelectionController::next_deadline_ms(
    std::uint32_t* deadline_ms) const {
  if (!active_ || deadline_ms == nullptr) {
    return false;
  }
  *deadline_ms = timeout_deadline_ms_;
  if (decision_deadline_ms_ != 0 &&
      static_cast<std::int32_t>(decision_deadline_ms_ - timeout_deadline_ms_) < 0) {
    *deadline_ms = decision_deadline_ms_;
  }
  return true;
}
