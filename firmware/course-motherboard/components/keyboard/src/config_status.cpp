...
void append_compact_power(std::ostringstream& out,
  ...
  // Awake is the only live state. Deep sleep cannot answer a status request,
  // so never serialize the retired Active/Idle/DeepIdle vocabulary even if a
  // stale caller supplies it during a mixed-version build.
  append_bounded_string_field(
      out, &first_power, "mode", "awake", kCompactStatusStringMaxLen);
  append_uint_field(out, &first_power, "inactive_ms", power.inactive_ms);
  append_bounded_string_field(out,
                              &first_power,
                              "deep_sleep_block",
                              power.deep_sleep_block,
                              24,
                              "unknown");
  append_bounded_string_field(out,
                              &first_power,
                              ...
                              kCompactStatusStringMaxLen,
                              "unknown");
  if (include_cycle && power.cycle_seq != 0) {
    append_uint_field(out, &first_power, "cycle_seq", power.cycle_seq);
    append_uint_field(
        out, &first_power, "cycle_inactive_ms", power.cycle_inactive_ms);
    append_bool_field(
        out, &first_power, "cycle_deep_sleep", power.cycle_deep_sleep);
    append_bounded_string_field(out,
                                &first_power,
                                "cycle_wake",
                                power.cycle_wake,
...
static std::string build_full_status_json(const ConfigStatusSnapshot& snapshot) {
  ...
  if (!snapshot.power.mode.empty()) {
    bool first_power = true;
    out << ",\"power\":{";
    append_string_field(out, &first_power, "mode", "awake");
    append_uint_field(out, &first_power, "inactive_ms", snapshot.power.inactive_ms);
    append_string_field(
        out, &first_power, "deep_sleep_block", snapshot.power.deep_sleep_block);
    ...
    if (snapshot.power.cycle_seq != 0) {
      ...
      append_uint_field(
          out, &first_power, "cycle_inactive_ms", snapshot.power.cycle_inactive_ms);
      append_bool_field(
          out, &first_power, "cycle_deep_sleep", snapshot.power.cycle_deep_sleep);
...
std::string build_config_status_json(const ConfigStatusSnapshot& snapshot) {
  ...
  if (!has_audio_status && !snapshot.diagnostics.board.empty()) {
    ...
    const auto without_cycle = build_compact_status_json(snapshot,
                                                          ...
                                                          false,
                                                          false);
    if (without_cycle.size() <= kConfigStatusGattSafeLen) {
      return without_cycle;
    }
