...
bool DebouncedInput::transition_pending() const {
  return initialized_ && candidate_state_ != stable_state_;
}

bool DebouncedInput::next_transition_deadline_ms(
    bool raw_state,
    std::uint32_t* deadline_ms) const {
  if (deadline_ms == nullptr || !transition_pending() ||
      raw_state != candidate_state_) {
    return false;
  }
  *deadline_ms = candidate_since_ms_ + interval_for(candidate_state_);
  return true;
}
