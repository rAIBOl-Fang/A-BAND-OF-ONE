...
ConfigParseStatus parse_action(std::string_view value, Action* action) {
  if (const auto named = string_value(value); named.has_value()) {
    ...
    if (!kind.has_value()) {
      // Not a known named action: a canonical host action value is accepted
      // with the full string preserved; anything else (including a
      // non-canonical host action value) fails closed without normalization.
      if (is_canonical_host_action_value(*named)) {
