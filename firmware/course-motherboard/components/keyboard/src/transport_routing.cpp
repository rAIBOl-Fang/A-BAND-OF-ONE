...
bool UsbPhysicalPresenceMonitor::next_update_deadline_ms(
    std::uint32_t* deadline_ms) const {
  if (!disconnect_pending_ || deadline_ms == nullptr) {
    return false;
  }
  *deadline_ms = disconnect_candidate_since_ms_ + disconnect_confirm_ms_;
  return true;
}
