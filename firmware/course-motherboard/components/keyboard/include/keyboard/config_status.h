...
struct PowerDiagnosticsSnapshot {
  ...
  // key `mode` for App compatibility, but callers must publish only `awake`.
  std::string mode;
  std::uint32_t inactive_ms = 0;
  std::string deep_sleep_block;
  std::string last_wake;
  std::uint32_t wake_edges = 0;
  bool usb = false;
  std::uint32_t cycle_seq = 0;
  std::uint32_t cycle_inactive_ms = 0;
  bool cycle_deep_sleep = false;
