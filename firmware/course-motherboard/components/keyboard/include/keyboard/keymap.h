...
// True when value is "host_action:" followed by exactly 36 ASCII characters
// of a canonical lowercase UUID: hyphens at 1-based positions 9/14/19/24 and
// lowercase hex digits everywhere else. Non-canonical input (uppercase, wrong
// length, misplaced hyphens, illegal characters) is rejected without
// normalization. No UUID version or nil restriction is applied.
bool is_canonical_host_action_value(std::string_view value);
