...
bool SpeakerAssetsRuntimeCore::next_deadline_ms(
    std::uint32_t now_ms,
  ...
  const auto consider = [&](std::uint32_t candidate) {
    const auto candidate_distance =
        static_cast<std::int32_t>(candidate - now_ms);
    const auto nearest_distance =
        static_cast<std::int32_t>(nearest - now_ms);
    if (!armed || candidate_distance < nearest_distance) {
      nearest = candidate;
      armed = true;
    }
  };
  ...
  if (session_partial_active_ &&
    ...
    consider(session_partial_last_activity_ms_ + partial_timeout_ms_);
  }
  if (session_lease_active_ && !pending_action_active_ &&
      !session_.action_pending() &&
      !route_has_ingress(session_lease_route_)) {
    consider(session_lease_last_activity_ms_ + session_lease_ms_);
  }
  if (armed) {
    *deadline_ms = nearest;
  }
