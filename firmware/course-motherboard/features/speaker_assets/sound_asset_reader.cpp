...
SoundAssetReadResult SoundAssetStreamDecoder::open_embedded(
  ...
  const bool loop_enabled =
      loop_start_frame != 0U || loop_end_frame != 0U;
  if (loop_enabled &&
  ...
  if (loop_enabled) {
    // EIAD frames are variable-length, so the loop start offset is resolved
    // by walking the frame headers once at open time. This stays far away
    // from the per-frame decode path.
    std::uint32_t offset = kEiadHeaderBytes;
    std::uint32_t samples = 0U;
    for (std::uint16_t index = 0U; index < loop_start_frame;
         ++index) {
      if (kSoundAssetFrameHeaderBytes >
          asset_.encoded_bytes - offset) {
        close();
        return SoundAssetReadResult::InvalidResource;
      }
      const std::uint16_t sample_count =
          read_le16(encoded + offset);
      ...
      if (sample_count == 0U ||
          ...
          frame_bytes > asset_.encoded_bytes - offset) {
        close();
        return SoundAssetReadResult::InvalidResource;
      }
      offset += frame_bytes;
    ...
    loop_start_frame_ = loop_start_frame;
    loop_end_frame_ = loop_end_frame;
    loop_start_offset_ = offset;
    loop_start_decoded_samples_ = samples;
  }
  return reset();
}
...
void SoundAssetStreamDecoder::close() {
  ...
  loop_end_frame_ = 0U;
  loop_start_offset_ = 0U;
  loop_start_decoded_samples_ = 0U;
  ready_ = false;
...
SoundAssetReadResult SoundAssetStreamDecoder::decode_next(
  ...
  *output_samples = sample_count;
  if (loop_end_frame_ != 0U && next_frame_index_ == loop_end_frame_) {
    // Sustain wrap: the decoded frame just consumed the end of the loop
    // window, so the stream restarts from the pre-resolved loop start
    // state instead of advancing towards End.
    next_encoded_offset_ = loop_start_offset_;
    decoded_samples_ = loop_start_decoded_samples_;
    next_frame_index_ = loop_start_frame_;
  }
  return SoundAssetReadResult::Ok;
}
